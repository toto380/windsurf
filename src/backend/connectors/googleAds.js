...
class GoogleAdsConnector {
  ...
  async fetchData() {
    if (!this.csvFilePath) {
      return {
        ...
        evidence: 'Google Ads CSV file not selected',
        confidence: 'LOW'
      };
    }
    ...
    try {
      ...
      if (!exists) {
        return {
          ...
          evidence: `Google Ads CSV file not found at ${this.csvFilePath}`,
          confidence: 'LOW'
        };
      }
      ...
      if (lines.length < 2) {
        return {
          ...
          evidence: 'Google Ads CSV file has no data',
          confidence: 'LOW'
        };
      }
      ...
      return {
        status: 'ok',
        customerId: 'CSV Import',
        dateRange: 'From CSV',
        metrics,
        topCampaigns,
        topAdGroups: [], // Can be extracted if needed
        topSearchQueries: [], // Can be extracted if needed
        evidence: `Data imported from Google Ads CSV file: ${path.basename(this.csvFilePath)}`,
        confidence: 'HIGH'
      };

    } catch (error) {
      console.error('[GoogleAds] CSV import error:', error.message);
      return {
        status: 'error',
        error: error.message,
        evidence: `Failed to parse Google Ads CSV file: ${path.basename(this.csvFilePath)}`,
        confidence: 'LOW'
      };
    }
  }
  ...
  _extractMetrics(headers, dataRows) {
    ...
    const conversionsIndex = headers.findIndex(h => h.toLowerCase().includes('conversion'));
    const valueIndex = headers.findIndex(h => h.toLowerCase().includes('value') || h.toLowerCase().includes('revenue'));

    // Sum up values from all rows
    dataRows.forEach(row => {
      if (costIndex >= 0) totalCost += parseFloat(row[costIndex]) || 0;
  ...
  _extractTopCampaigns(headers, dataRows) {
    ...
    dataRows.forEach(row => {
      ...
      if (costIndex >= 0) campaigns[campaignName].cost += parseFloat(row[costIndex]) || 0;
    ...
    return Object.values(campaigns)
      ...
      .map(campaign => ({
        ...campaign,
        cost: Math.round(campaign.cost * 100) / 100,
  ...
  static async importFromCSV(csvFilePath, periodDays = null) {
    const connector = new GoogleAdsConnector(csvFilePath, periodDays);
  ...
  static async testConnection(csvFilePath) {
    ...
    try {
      const connector = new GoogleAdsConnector(csvFilePath);
